...
Usa únicamente [GitHub Releases](https://github.com/JotaEse68/jota-ai-launcher/releases/latest). Descarga el instalador `.exe` para Windows o `.dmg` para macOS y `SHA256SUMS.txt` de la misma release.

## 2. Compara el SHA-256

En Windows, abre PowerShell en la carpeta de descargas:

```powershell
Get-FileHash -Algorithm SHA256 ".\Jota-AI-Launcher-Setup-0.2.0.exe"
...
```shell
shasum -a 256 Jota-AI-Launcher-0.2.0-universal.dmg
cat SHA256SUMS.txt
```
...
```powershell
gh attestation verify ".\Jota-AI-Launcher-Setup-0.2.0.exe" --repo JotaEse68/jota-ai-launcher
```

En macOS, sustituye el nombre por `Jota-AI-Launcher-0.2.0-universal.dmg`.
...
```powershell
Update-MpSignature
Start-MpScan -ScanType CustomScan -ScanPath (Resolve-Path ".\Jota-AI-Launcher-Setup-0.2.0.exe")
```
...
En macOS también puedes comprobar el archivo con el antivirus que utilices antes de montarlo.

## Sobre SmartScreen y Gatekeeper

La aplicación no tiene todavía certificados comerciales de firma de código. Windows puede mostrar **Editor desconocido** y macOS Gatekeeper puede bloquear la primera apertura. Comprueba el origen, el hash, la atestación y el análisis antivirus antes de decidir si la ejecutas. El objetivo futuro es firmar y notarizar ambos instaladores.
